...
if [ -z "${BEST_CKPT}" ]; then
    BEST_CKPT=$(find "${TRAINING_DIR}/lightning_logs" -name "*.ckpt" -type f 2>/dev/null | sort | tail -1)
fi
...
ONNX_OUTPUT="${EXPORT_DIR}/si_LK-medium.onnx"
...
CONFIG_DST="${EXPORT_DIR}/si_LK-medium.onnx.json"
...
aws s3 cp "${ONNX_OUTPUT}" \
    "s3://${S3_BUCKET}/${S3_CHECKPOINT_PREFIX}/exported/si_LK-medium.onnx" \
    --region "${S3_REGION}"

if [ -f "${CONFIG_DST}" ]; then
    aws s3 cp "${CONFIG_DST}" \
        "s3://${S3_BUCKET}/${S3_CHECKPOINT_PREFIX}/exported/si_LK-medium.onnx.json" \
        --region "${S3_REGION}"
fi
...
echo "  echo 'ආයුබෝවන්' | piper --model ${ONNX_OUTPUT} --output_file test.wav"
echo "  # or download from S3:"
echo "  aws s3 cp s3://${S3_BUCKET}/${S3_CHECKPOINT_PREFIX}/exported/si_LK-medium.onnx ."
echo "  aws s3 cp s3://${S3_BUCKET}/${S3_CHECKPOINT_PREFIX}/exported/si_LK-medium.onnx.json ."
echo "  echo 'ආයුබෝවන්' | piper --model si_LK-medium.onnx --output_file test.wav"
